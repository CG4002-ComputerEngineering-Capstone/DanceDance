...
def checksum_imu(packet):
    check = packet[0] ^ packet[1] ^ packet[2] ^ packet[3] ^ packet[4] ^ packet[
        5] ^ packet[6] ^ packet[7] ^ packet[8] ^ packet[9] ^ packet[10]
    if check == packet[11]:
        return True
    return False
                ...
                if(correct_size):
                    ...
                    if packet_type == 1:
                        ...
                        if (checksum_ack(packet)):
                            ...
                            print(Connect_Header)
                            print(
                                "Successfully Received ACK packet from beetle ID ", i)
...
class Setup:
    def setPeripheral(address):
        try:
            beetle = btle.Peripheral(
                address, addrType=btle.ADDR_TYPE_PUBLIC, iface=0)
        except:
...
class main_thread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            ...
            global_delegate[self.ID] = self.beetle.setDelegate(
                MyDelegate(self.beetle.addr))
