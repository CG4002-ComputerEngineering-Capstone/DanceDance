...
class Server(threading.Thread):
    def __init__(self, ip_addr, port_num, group_id, n_moves=len(ACTIONS) * NUM_MOVE_PER_ACTION):
        ...
        self.columns = ['timestamp', 'position', 'gt_position', 'action', 'gt_action',
                        'response_time', 'sync', 'is_action_correct', 'is_position_correct']
    ...
    def decrypt_message(self, cipher_text):
        ...
        print('[Position, Action, Sync]:', position, action, sync)
        return {
    ...
    def setup_connection(self):
        ...
        if len(secret_key) == 16 or len(secret_key) == 24 or len(secret_key) == 32:
            ...
            print(secret_key)
            print(f'AES key is length {len(secret_key)}')
    ...
    def set_next_action(self):
        ...
        print('[Next action]:', POSITIONS[position])
...
def main():
    ...
    display_window = Tk()
    action_display = add_display_label(
        display_window, label=str(my_server.action))
    position_display = add_display_label(
        display_window, label=str(my_server.position))
